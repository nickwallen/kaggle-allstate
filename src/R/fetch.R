...
fetch <- function (path = "../../data/train.csv.zip") {
  ...
  # rename the option 'a' to 'option.a'
  options.old <- na.omit (str_extract (names (train), "^[a-g]$"))
  options.new <- paste0 ("option.", options.old)
  setnames (train, options.old, options.new)
  
  # clean-up the data types
  train [, `:=` (
    
    # can't assume a customer ID is truly numeric
