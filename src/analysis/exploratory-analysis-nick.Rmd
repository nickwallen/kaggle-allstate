...
library (reshape2)
train.melted <- train [, list (customer.id, cost, option.a, option.b, option.c, option.d, option.e, option.f, option.g)]
train.melted <- data.table (melt (train.melted, id.vars = c("customer.id", "cost")))

# TODO - why is this not working?
# transform the data from wide to long for plotting
#train.melted <- train [, list (customer.id, cost, option.a, option.b, option.c, option.d, option.e, option.f, option.g)]
#train.melted <- train [, list ( option.name  = as.factor (names (.SD)), option.value = unlist(.SD, use.names = F)), by = list (customer.id, cost) ]

limits <- quantile (train.melted$cost, c(0.01, 0.99))

p <- ggplot (train.melted, aes (value, cost, fill = value))
p + geom_boxplot() + scale_y_continuous (limits = limits) + facet_wrap (~ variable, ncol = 4, scales="free")
```
