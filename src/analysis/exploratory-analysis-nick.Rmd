...
# transform the set of options from wide to long
train.melted <- train [, list (customer.id, cost, option.a, option.b, option.c, 
                               option.d, option.e, option.f, option.g)]
train.melted <- data.table (melt (train.melted, id.vars = c("customer.id", "cost")))

limits <- quantile (train.melted$cost, c(0.01, 0.99))

# plot a comparison of options to total product cost
boxplot <- ggplot (train.melted, aes (value, cost, fill = value)) + geom_boxplot() 
boxplot + scale_y_continuous (limits = limits) + facet_wrap (~variable, ncol = 4, scales="free") + guides(fill=F)
```
...
### Q. Does group size have a big impact on cost?  

It appears that group size has very little impact on the total cost reflected in the data.

```{r}
p <- ggplot (train, aes (as.factor(group.size), cost, fill = as.factor(group.size))) 
p + geom_violin() + geom_boxplot (aes (fill=NULL), alpha = 0.1, width = 0.1, outlier.shape=NA) 
```
