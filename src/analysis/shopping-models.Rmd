...
naive.models <- train.naive.model (train)
naive.predictions <- predict.naive.model (naive.models, train)

# make a prediction for each option [a-g] using the correct model
for (option.hat in names (models)) {
  train.shopping [, option.hat := predict (models [[ option.hat ]], .SD), .SDcols = 2:23, with = FALSE ]
}
