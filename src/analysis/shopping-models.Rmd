...
Create a partial function to populate some of the required parameters that won't change

```{r}
# create a partial function to make creating multiple models simpler
train.shopping <- function (method, data, ..., option = "option.a") {
...
A tree-based model that uses pseudo-regression where options are treated as numerics.
...
# treat each of the option as numeric; pseudo-regression
options.as.numeric (shopping.train)
optoins.as.numeric (shopping.test)

# tree-based pseudo-regression where options are numerics
gbm.grid <- expand.grid (interaction.depth = c(1, 5, 9), n.trees = c(50, 100, 150, 200), shrinkage = 0.1)
fit.gbm.regress <- train.shopping ("gbm", shopping.train, tuneGrid = gbm.grid)

# make a prediction
shopping.test [, option.a.hat := round (predict (fit.gbm.regress, .SD)), .SDcols = 2:23 ]

result.1 <- with (shopping.test, confusionMatrix(option.a.hat, option.a))
```

A tree-based classification model where each option has a fixed set of possible classes.
...
options.as.factors (shopping.train)
options.as.factors (shopping.test)

# tree-based classification
gbm.grid <- expand.grid (interaction.depth = c(1, 5, 9), n.trees = c(50, 100, 150, 200), shrinkage = 0.1)
fit.gbm.class <- train.shopping ("gbm", shopping.train, tuneGrid = gbm.grid)

# make a prediction
shopping.test [, option.a.hat := predict (fit.gbm.class, .SD), .SDcols = 2:23 ]

result.2 <- with (shopping.test, confusionMatrix(option.a.hat, option.a))

```

A tree-based classification model where each option has a fixed set of possible classes.  Using a mean over a sum.

```{r}

# flatten the source into a form that can be used for training
shopping <- extract.shopping.history (data, mean)
...
# tree-based classification
gbm.grid <- expand.grid (interaction.depth = c(5, 9), n.trees = c(100, 150), shrinkage = 0.1)
fit.gbm.class <- train.shopping ("gbm", shopping.train, tuneGrid = gbm.grid)

# make a prediction
shopping.test [, option.a.hat := predict (fit.gbm.class, .SD), .SDcols = 2:23 ]

result.3 <- with (shopping.test, confusionMatrix(option.a.hat, option.a))
