...
Lastly, there is a group of customers for which we have no shopping history.  For these customers we could use a purely customer based model.
...
# split the competition training data set 
train.index <- createDataPartition (data$plan, p = 0.8, list = F)[,1]
train <- data [  train.index ] 
test  <- data [ -train.index ]
...
# the model does not require training, hence it only uses the test data
...
train.shopping <- extract.shopping.history (train)
train.shopping <- extract.purchase.history (train, train.shopping)

# train the models
models <- cache ("train.alpha.models", 
                  train.alpha.model (train.shopping, verbose = TRUE))

# TODO - 19393 of the option values trying to predict are NA, which causes this to blow up... this is because some of the customers have no shopping history.  this situation needs to be handled in train.alpha.model

# prepare the test data
test.shopping <- extract.shopping.history (test)
test.shopping <- extract.purchase.history (test, test.shopping)

# predict the future
predict.alpha.model (models, test.shopping)
